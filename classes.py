...
class msgresponse:
    def check_func(self, number, message):
        ...
        if (fuzz.ratio(message.lower(), "stop")) > 95:
        ...
        # ----------------if message was not "stop",execute else block----------------------------------
        else:
            ...
            try:
                ...
                with open("states_dist.csv", "r") as csvFile:
                    ...
                    for row in self.reader:
                        self.prob_state = fuzz.ratio(self.msg.lower(), row[0].lower())

                        # ---if state name in message was close to any of in saved names--
                        if self.prob_state > 90:
                            for ind in self.df.index:
                        ...
                        return "please check your state spelling and resend again\r\n*example:* if your state is Gujarat, send *Gujarat* as message"
            except:
